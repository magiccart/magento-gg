 ...
 * @@Modify Date: 2015-12-28 10:14:22
 * @@Function:
 */
 ?>
<?php
		...
		if ($data = $this->getRequest()->getPost()) {
			...
			if($id){
				...
				if($check->getConfigId() != $id){
	                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('magicproduct')->__('Identifier can not change')); //Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
				...
				if(!isset($data['image'])){
					...
					if($tmp && isset($tmp['image'])) $data['image'] = $tmp['image'];
					$value = serialize($data);
				}
