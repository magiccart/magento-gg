 ...
 * @@Modify Date: 2015-08-03 16:01:54
 * @@Function:
 */
	...
	(function(selector){
	    ...
	    if($navDesktop.hasClass('sticker')){			
			$(window).scroll(function () {
			 if ($(this).scrollTop() > 80) $('.header-bottom').addClass('header-container-fixed');
			 else $('.header-bottom').removeClass('header-container-fixed');
			 return false;
			});
	    }
	    // End Sticker Menu
		...
		$navtop.each(function(index, val) {
			...
			if(options){
				...
				var $block_left = $('.mega-block-left', $item).width(left_width + '%');
				var $block_right = $('.mega-block-right', $item).width(right_width + '%');
